...
**选型结论（面试可讲的关键判断）**：四个模型 ROC-AUC 差距在 1 个百分点左右，XGBoost 并未显著优于逻辑回归——因为该数据集信号以近似线性的加性效应为主、样本量中等、独热后特征维度低。综合**可解释性**（系数可直接用于业务解读与监管解释）、**训练/部署成本**与效果，最终上线逻辑回归。这比"无脑上复杂模型"更接近真实工业界的决策方式。
...
> **电信用户流失预测系统**｜Python / scikit-learn / XGBoost / Streamlit
>
> - 基于 7,043 条电信用户数据完成二分类建模，覆盖数据清洗、特征工程、建模评估到部署演示的完整 ML 生命周期；
> - 清洗 TotalCharges 脏值并构造服务绑定数、月均支出等 4 个业务衍生特征，基于 Pipeline + ColumnTransformer 实现防泄漏的标准化与独热编码；
> - 针对 26.6% 的类别不平衡，对比 class_weight 加权与 SMOTE 过采样两种方案及 LR/RF/XGBoost 三类模型，最优模型 **ROC-AUC 0.87、PR-AUC 0.73、流失用户召回率 0.77**；
> - 基于可解释性与部署成本完成模型选型，使用 Streamlit 部署交互页面，支持单用户流失概率预测与高风险用户名单 CSV 导出，并通过阈值调节量化召回-营销成本权衡。

## 八、面试可能追问的问题

1. **SMOTE 为什么要放在 Pipeline 里？** —— 保证重采样只在训练折内发生；若先 SMOTE 再划分，验证集会出现由训练样本插值合成的"近亲样本"，指标虚高（数据泄漏）。
2. **ROC-AUC 和 PR-AUC 的区别？** —— 前者对类别比例不敏感，不平衡严重时显得过度乐观；后者聚焦正类的精确率-召回率，更能反映少数类识别能力。
3. **加权和过采样怎么选？** —— 加权不改变数据分布、不引入合成样本噪声，是首选；SMOTE 在少数类决策边界过于稀疏时可尝试，需用交叉验证确认收益。
4. **为什么不调阈值到召回 100%？** —— 召回与精确率此消彼长，阈值过低会把大量忠诚用户纳入挽留名单，营销成本可能超过挽回收入；应结合单次挽留成本与用户 LTV 反推阈值。
5. **模型如何上线？** —— 本项目用 joblib 持久化含预处理器的完整 Pipeline，Streamlit 仅作演示；生产化可包成 FastAPI 服务（预处理器在 Pipeline 内，入模特征与训练时严格一致），批量场景可对全量用户夜间打分写入名单表。
6. **如何进一步提升？** —— 交叉验证 + 网格/贝叶斯调参；引入 SHAP 做个体级解释；特征分箱（WOE/IV）；接入用户行为时序数据做生存分析（预测剩余生命周期）。
